...
// GET /api/clients - List and search clients
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (role === 'technician') {
      // Find technician's worker record
      ...
      // Technicians only see clients they've worked for
      const query = `
        SELECT DISTINCT c.*
        FROM clients c
      ...
      `;
      clients = await sql(query, [workerId, `%${search}%`]);
    } else {
      // super_admin and manager see all clients
      const query = `
        SELECT * FROM clients
        WHERE full_name ILIKE $1 OR phone ILIKE $1
        ORDER BY full_name ASC
      `;
      clients = await sql(query, [`%${search}%`]);
    }
...
export async function POST(req: NextRequest) {
  ...
  // Enforce role permission: technician cannot create clients
  if (role === 'technician') {
  ...
  try {
    ...
    const existing = await sql(`SELECT id FROM clients WHERE phone = $1 LIMIT 1`, [phone]);
