...
export function ClientPortalView({
  ...
  return (
    <div
      className="min-h-screen bg-surface-base text-text-primary antialiased selection:bg-blue-600 selection:text-white"
      dir={isRtl ? 'rtl' : 'ltr'}
    >
      {/* Top Ambient Glow */}
      ...
      {/* Top Header Bar */}
      <header className="sticky top-0 z-40 backdrop-blur-xl bg-surface-base/85 border-b border-border-subtle/80 px-4 py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold text-text-primary text-sm tracking-tight">{t.garageName}</span>
              ...
              <p className="text-[11px] text-text-muted font-medium">{t.digitalServiceBook}</p>
            </div>
          </div>
      ...
      <main className="max-w-2xl mx-auto px-4 pt-6 pb-24 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-1.5 bg-surface-raised/90 border border-border-subtle p-1.5 rounded-2xl backdrop-blur-xl shadow-xl">
          {[
            {
              id: 'history',
            ...
            <button
              ...
              className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-xl text-center transition-all ${
                activeTab === tab.id
                  ...
                  : 'text-text-muted hover:text-text-primary hover:bg-surface-overlay/40 font-medium'
              }`}
            >
        ...
        {activeTab === 'history' && (
          <div className="space-y-4">
            ...
            {filteredActions.length === 0 ? (
              <div className="bg-surface-raised/60 border border-border-subtle rounded-2xl p-8 text-center space-y-2">
                <svg className="w-8 h-8 text-text-disabled mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
                </svg>
                <p className="text-sm font-semibold text-text-secondary">{t.history.empty}</p>
                <p className="text-xs text-text-muted">Les interventions futures enregistrées par l&apos;atelier s&apos;afficheront ici.</p>
              </div>
            ) : (
              <div className="space-y-4 relative before:absolute before:top-3 before:bottom-3 before:left-[15px] rtl:before:right-[15px] rtl:before:left-auto before:w-[2px] before:bg-surface-overlay">
                {filteredActions.map((act) => (
                  <div key={act.id} className="relative pl-8 rtl:pr-8 rtl:pl-0 group">
                    {/* Node Dot */}
                    <span className="absolute left-[9px] rtl:right-[9px] rtl:left-auto top-4 h-3.5 w-3.5 rounded-full bg-blue-600 border-2 border-surface-base" />

                    <div className="bg-surface-raised/90 border border-border-subtle rounded-2xl p-5 shadow-xl transition space-y-3">
                      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border-subtle pb-3">
                        <div>
                          ...
                          <h3 className="text-base font-bold text-text-primary capitalize mt-0.5">
                        ...
                        <span className="bg-surface-base border border-border-subtle px-2.5 py-1 rounded-lg text-xs font-mono text-blue-400 font-bold">
                      ...
                      {act.client_visible_notes && (
                        <div className="p-3 rounded-xl bg-surface-base/60 border border-border-subtle text-xs text-text-muted space-y-1">
                          <span className="font-bold text-text-secondary block">{t.history.notes}</span>
        ...
        {activeTab === 'booking' && (
          <div className="space-y-6">
            {/* Booking Form Card */}
            <div className="bg-surface-raised/90 border border-border-subtle rounded-2xl p-6 shadow-xl space-y-5">
              <div>
                <h2 className="text-lg font-bold text-text-primary">{t.booking.title}</h2>
                <p className="text-xs text-text-muted mt-0.5">{t.booking.subtitle}</p>
              </div>
              ...
              <form onSubmit={handleBookingSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-surface-base border border-border-subtle focus:border-blue-500 rounded-xl px-3.5 py-3 text-text-primary text-sm outline-none transition"
                    required
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-surface-base border border-border-subtle focus:border-blue-500 rounded-xl px-3.5 py-2.5 text-text-primary text-sm outline-none transition"
                      required
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-surface-base border border-border-subtle focus:border-blue-500 rounded-xl px-3.5 py-2.5 text-text-primary text-sm outline-none transition font-mono"
                    />
                  </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-surface-base border border-border-subtle focus:border-blue-500 rounded-xl px-3.5 py-2.5 text-text-primary text-sm outline-none transition resize-none"
                  />
                </div>
            ...
            {appointmentsList.length > 0 && (
              <div className="bg-surface-raised/90 border border-border-subtle rounded-2xl p-6 shadow-xl space-y-4">
                <h3 className="text-base font-bold text-text-primary">{t.booking.myAppointments}</h3>
                <div className="space-y-3">
                  {appointmentsList.map((app) => (
                    <div key={app.id} className="p-4 rounded-xl bg-surface-base border border-border-subtle space-y-2">
                      ...
                      {app.garage_response && (
                        <p className="text-xs text-text-secondary bg-surface-raised/80 p-2.5 rounded-lg border border-border-subtle">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-surface-raised/90 border border-border-subtle rounded-2xl p-6 shadow-xl space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                <div className="p-3 rounded-xl bg-surface-base border border-border-subtle">
                  <span className="text-text-muted font-bold block">{t.engine}</span>
                  <span className="text-text-primary font-semibold block mt-0.5">{vehicle.engine_spec || 'Standard'}</span>
                </div>
                <div className="p-3 rounded-xl bg-surface-base border border-border-subtle">
                  <span className="text-text-muted font-bold block">{t.fuel}</span>
                  <span className="text-text-primary font-semibold block mt-0.5">{vehicle.fuel_type || 'Diesel'}</span>
                </div>
                <div className="p-3 rounded-xl bg-surface-base border border-border-subtle">
                  <span className="text-text-muted font-bold block">{t.transmission}</span>
                  <span className="text-text-primary font-semibold block mt-0.5">{vehicle.transmission || 'Manuelle'}</span>
                </div>
                <div className="p-3 rounded-xl bg-surface-base border border-border-subtle">
                  <span className="text-text-muted font-bold block">{t.oil}</span>
                  <span className="text-blue-400 font-semibold font-mono block mt-0.5">{vehicle.oil_type || '5W-30 ACEA C3'}</span>
                </div>
                <div className="p-3 rounded-xl bg-surface-base border border-border-subtle col-span-2 sm:col-span-1">
                  <span className="text-text-muted font-bold block">{t.tires}</span>
                  <span className="text-text-primary font-semibold font-mono block mt-0.5">{vehicle.tire_size || '—'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-surface-raised/90 border border-border-subtle rounded-2xl p-6 shadow-xl space-y-4">
              ...
              <div className="space-y-3 text-xs">
                {[
                  ...
                  <div key={idx} className="p-3.5 rounded-xl bg-surface-base border border-border-subtle flex items-start justify-between gap-3">
                    <div>
            ...
            <div className="bg-surface-raised/90 border border-border-subtle rounded-2xl p-6 shadow-xl space-y-4">
              <div>
                ...
                <p className="text-xs text-text-muted mt-0.5">Identification des alertes du tableau de bord et procédures de sécurité.</p>
              </div>
              ...
              <div className="space-y-2.5">
                <input
                  ...
                  className="w-full bg-surface-base border border-border-subtle focus:border-blue-500 rounded-xl px-3.5 py-2.5 text-xs text-text-primary outline-none"
                />
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg font-bold transition shrink-0 ${
                        warningColorFilter === col
                          ? 'bg-blue-600 text-white'
                          : 'bg-surface-base border border-border-subtle text-text-muted'
                      }`}
                    >
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <h4 className="font-bold text-text-primary text-xs sm:text-sm">{w.name}</h4>
                        ...
                        <p className="text-xs text-text-secondary leading-relaxed">{w.description}</p>
                        <p className="text-[11px] text-blue-400 font-semibold pt-1">
                          <span className="text-text-muted">Action requise :</span> {w.action}
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        {activeTab === 'reminders' && (
          <div className="space-y-6">
            <div className="bg-surface-raised/90 border border-border-subtle rounded-2xl p-6 shadow-xl space-y-5">
              <div>
                <h2 className="text-lg font-bold text-text-primary">{t.reminders.title}</h2>
                <p className="text-xs text-text-muted mt-0.5">{t.reminders.subtitle}</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className={`p-5 rounded-2xl border space-y-3 transition ${
                  ...
                  <div className="flex items-start justify-between">
                    <span className="text-xs font-bold text-text-muted uppercase tracking-wider">{t.reminders.nextOilChange}</span>
                    <svg className="w-5 h-5 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <div>
                    <span className="text-2xl font-black font-mono text-text-primary block">
                ...
                <div className="p-5 rounded-2xl bg-surface-base border border-border-subtle space-y-3">
                  <div className="flex items-start justify-between">
                    <span className="text-xs font-bold text-text-muted uppercase tracking-wider">{t.reminders.nextCT}</span>
                  ...
                  <a
                    ...
                    download
                    className="w-full py-2.5 px-3 rounded-xl bg-surface-overlay hover:bg-surface-hover text-text-primary text-xs font-bold flex items-center justify-center gap-2 transition"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
              ...
              <div className="p-4 rounded-2xl bg-surface-base border border-border-subtle flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="space-y-0.5">
                  <h4 className="font-bold text-sm text-text-primary">{t.reminders.whatsappRemind}</h4>
                  <p className="text-xs text-text-muted">Demande d&apos;assistance ou précision technique directe avec les mécaniciens.</p>
                </div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-4 py-2.5 rounded-xl bg-surface-overlay hover:bg-surface-hover text-text-primary text-xs font-bold shrink-0 transition flex items-center gap-1.5"
                >
